...
for d in all_devices_list:
    ...
    commands = ["sudo docker run --ulimit nofile=500:500  -d -v /sys/fs/cgroup:/sys/fs/cgroup:ro -v {0}:{1} --cpus={2} --memory={5}m --storage-opt size={6}M --privileged --cap-add=NET_ADMIN --cap-add=NET_RAW --hostname {3} --name {3} {4}".format(vm_mount_path,container_host_mount_path,cpus,d,container_OS,memory_mb,disk_mb)]
