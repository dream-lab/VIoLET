...
deployment_output = json.load(open("../../infra/deployment_output.json"))
eth_ip_dict = json.load(open("../../infra/eth_ip.json"))
device_cpus_updated = json.load(open("{0}".format(file_name)))

private_networks_dict = infra_config["private_networks"]
public_networks_dict = infra_config["public_networks"]
all_devices_list = device_cpus_updated.keys()
...
f_pi2b_delta = open("logs/f_pi2b_delta", "w")
f_pi3b_delta = open("logs/f_pi3b_delta", "w")
f_tx1_delta = open("logs/f_tx1_delta", "w")
f_si_delta  = open("logs/f_si_delta", "w")

f_pi2b = open("logs/f_pi2b", "w")
f_pi3b = open("logs/f_pi3b", "w")
f_tx1 = open("logs/f_tx1", "w")
f_si = open("logs/f_si", "w")
f_cm_all = open("logs/f_cm_all","w")
...
for device in all_devices_list:
    ...
    expected_coremark = device_cpus_updated[device]["coremark"]
...
with open('logs/cm_device_all','w') as file:
