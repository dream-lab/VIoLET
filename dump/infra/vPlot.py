...
f_percentile = open("../percentile","a")
...
for i in range(0,len(inputFile.columns)):
    ...
    percentile.append(np.percentile([inputFile[i].dropna().tolist()], 99, axis=1)[0])
...
#Writing Percentile values
for i in range(1,len(percentile) + 1):
    text(i,percentile[i-1],'%.1f' % percentile[i-1],horizontalalignment='left',color='magenta')
    print "percentile[i-1]={0} .1f' % percentile[i-1]={1}".format(percentile[i-1],'%.1f' % percentile[i-1])
    f_percentile.write("{0}\n".format('%.1f' % percentile[i-1]))
...
#inds = np.arange(1, len(medians) + 1)
#ax.annotate(quantile99,(0,0),(0,20), xycoords='axes fraction', textcoords='offset points', va='top')
